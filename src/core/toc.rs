...
use crate::core::{NodeId, VectorlessTree, VectorlessNode};
...
impl TocView {
    ...
    pub fn generate_from(&self, tree: &VectorlessTree, start: NodeId) -> TocNode {
        self.build_toc_node(tree, start, 0)
    }

    /// Generate flat ToC (all nodes in depth-first order).
    ...
    /// Generate filtered ToC (only nodes matching criteria).
    pub fn generate_filtered<F>(&self, tree: &VectorlessTree, filter: F) -> Vec<TocNode>
    where
        F: Fn(&VectorlessNode) -> bool,
    {
        let mut results = Vec::new();
        self.collect_filtered(tree, tree.root(), &filter, &mut results);
        results
    }

    /// Build ToC node recursively.
    fn build_toc_node(&self, tree: &VectorlessTree, node_id: NodeId, current_depth: usize) -> TocNode {
        let node = match tree.get(node_id) {
            Some(n) => n,
            None => return TocNode::new("(unknown)", current_depth),
        };

        // Check depth limit
        if let Some(max) = self.config.max_depth {
            if current_depth > max {
                return TocNode::new(&node.title, current_depth);
            }
        }

        // Build ToC node
        let mut toc = TocNode::new(&node.title, current_depth);

        if let Some(ref id) = node.node_id {
            toc = toc.with_node_id(id);
        }

        if self.config.include_pages {
            if let (Some(start), Some(end)) = (node.start_page, node.end_page) {
                toc = toc.with_page_range(start, end);
            }
        }

        if self.config.include_summaries && !node.summary.is_empty() {
            toc = toc.with_summary(&node.summary);
        }

        // Process children
        for child_id in tree.children(node_id) {
            let child_toc = self.build_toc_node(tree, child_id, current_depth + 1);
            toc.add_child(child_toc);
        }
    ...
    fn collect_filtered<F>(&self, tree: &VectorlessTree, node_id: NodeId, filter: &F, results: &mut Vec<TocNode>)
    where
        F: Fn(&VectorlessNode) -> bool,
    {
        if let Some(node) = tree.get(node_id) {
            if filter(node) {
                let toc = self.build_toc_node(tree, node_id, node.depth);
                results.push(toc);
            }

            for child_id in tree.children(node_id) {
                self.collect_filtered(tree, child_id, filter, results);
            }
        }
    }
    ...
    pub fn format_markdown(&self, toc: &TocNode) -> String {
        ...
        self.format_markdown_recursive(toc, &mut output);
        output
    }

    fn format_markdown_recursive(&self, toc: &TocNode, output: &mut String) {
        let indent = "  ".repeat(toc.depth);
        let bullet = if toc.children.is_empty() { "-" } else { "+" };
        ...
        for child in &toc.children {
            self.format_markdown_recursive(child, output);
        }
    }
